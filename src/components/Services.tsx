...
export default function ServicesSection() {
  return (
    ...
    >
      ...
      <div className="absolute left-6 bottom-0 pointer-events-none opacity-70">
        <Image
      ...
      <div className="absolute right-6 bottom-0 pointer-events-none opacity-70">
        <Image
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {services.map((s) => (
            <Link key={s.title} href={s.href} className="block group h-full">
              <article className="bg-white p-8 shadow-2xl border border-transparent transition-transform duration-300 transform hover:scale-105 hover:shadow-3xl hover:border-transparent relative flex flex-col items-center text-center cursor-pointer h-full">
                {/* Icon block */}
                <div className="mb-6">
                  <div className="w-20 h-20 rounded-lg border-2 border-gray-100 flex items-center justify-center transition-colors duration-300 group-hover:border-[#009FB2]">
                    <span className="text-gray-800 group-hover:text-[#009FB2]">
                      {s.icon}
                    </span>
                  </div>
                </div>
                ...
                {/* Title */}
                <h3 className="text-lg md:text-xl font-semibold text-gray-900 mb-4 group-hover:text-[#009FB2]">
                  {s.title}
                </h3>

                {/* List */}
                <ul className="space-y-2 text-sm text-gray-600 text-left flex-1">
                  {s.items.map((it) => (
                    <li key={it} className="flex items-start gap-3">
                      <Check className="w-4 h-4 mt-1 text-[#009FB2] flex-shrink-0" />
                      <span>{it}</span>
                    </li>
                  ))}
                </ul>
